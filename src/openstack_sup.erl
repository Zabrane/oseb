...
%%% @doc OpenStack supervisor. This module starts the openstack_records server and
%%%      restarts the server if it crashes.
%%% @end

-module(openstack_sup).
...
%% @private
%% @doc Supervisor callback. Starts the openstack server.
-spec(init(integer()) -> {ok, {RestartStrategy::tuple(), Servers::list(module())}}).
init(EtsTableId) ->
    Server = {openstack_records, {openstack_records, start_link, [EtsTableId]},
	      permanent, 2000, worker, [openstack_records]},
